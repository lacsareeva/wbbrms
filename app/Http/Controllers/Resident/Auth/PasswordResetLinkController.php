...
class PasswordResetLinkController extends Controller
{
    ...
    public function sendOtp(Request $request)
    {
        ...
        return redirect()->route('resident.verify-otp')->with('message', 'OTP has been sent to your email.');
    }
    ...
    public function verifyOtp(Request $request)
    {
        ...
        if (!$email) {
            return redirect()->route('resident.auth.verify-otp')->withErrors(['email' => 'Session expired or email not found.']);
        }
        ...
        if ($otpRecord) {
            ...
            return redirect()->route('resident.password.reset', ['email' => $otpRecord->email])->with('message', 'otp verification successfull.');
        }
    ...
    {
        return view('resident.auth.reset-password', ['email' => $email]);
    }
    ...
    public function resetPassword(Request $request)
    {
        ...
        if ($user) {
            ...
            return redirect()->route('resident.login')->with('message', 'Password has been successfully reset.');
        }
